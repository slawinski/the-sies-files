...
import { storytellerSetCondition } from "@/modules/scenario/scenario.service";

const schema = z.object({
  commandId: z.string().min(1),
  expectedVersion: z.number().int().nonnegative(),
  payload: z.object({ conditionId: z.string().min(1), active: z.boolean() }),
});
...
export async function POST(req: Request, { params }: { params: Promise<{ gameId: string }> }) {
  try {
    ...
    const { version } = await storytellerSetCondition({
      gameId,

...
import { closeInvestigation } from "@/modules/investigation/investigation.service";
...
export async function POST(req: Request, { params }: { params: Promise<{ gameId: string }> }) {
  try {
    ...
    const { version, winner } = await closeInvestigation({ gameId, commandId, expectedVersion });
